...
export interface ListProjectIdentityResponse {
  data: Pick<
    ProjectIdentity,
    'id' | 'projectId' | 'type' | 'email' | 'domain' | 'status' | 'createdAt' | 'updatedAt'
  >[];
  totalCount: number;
  totalPages: number;
  currPage: number;
...
export interface ListProjectIdentityQuery {
  currPage: number;
  perPage: number;
}

export interface ListProjectIdentityRequest
  extends RouteParams<
    any,
    ListProjectIdentityQuery,
    {
      projectId: string;
    }
  > {}

async function validate(params: ListProjectIdentityRequest) {
...
async function handle(params: ListProjectIdentityRequest) {
  ...
  return json<ListProjectIdentityResponse>({
    data: identities,
    totalCount,
    totalPages,
    currPage,
...
export const GET: APIRoute = handler(
  handle satisfies HandleRoute<ListProjectIdentityRequest>,
  validate satisfies ValidateRoute<ListProjectIdentityRequest>,
  {
    isProtected: true,
  },
);
